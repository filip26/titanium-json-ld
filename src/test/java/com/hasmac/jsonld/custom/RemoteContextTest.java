 ...
 */
package com.hasmac.jsonld.custom;
...
import com.hasmac.jsonld.JsonLd;
import com.hasmac.jsonld.JsonLdError;
import com.hasmac.jsonld.JsonLdOptions;
...
import com.hasmac.jsonld.json.JsonLdComparison;
import com.hasmac.jsonld.loader.ClasspathLoader;
import com.hasmac.rdf.RdfComparison;
...
class RemoteContextTest {
    ...
    void testToRdfMissingTriples1() throws JsonLdError, IOException {
        ...
        final Document document = readDocument("/com/hasmac/jsonld/test/issue61-in.json");
        final Document context = readDocument("/com/hasmac/jsonld/test/issue61-context.json");

        final RdfDataset result = JsonLd.toRdf(document).context(context).get();

        assertNotNull(result);

        try (final InputStream is = getClass().getResourceAsStream("/com/hasmac/jsonld/test/issue61-out.nq")) {
    ...
    void testRemoteContext() throws JsonLdError, IOException {
        ...
        final Document document = readDocument("/com/hasmac/jsonld/test/issue63-in.json");

        final RdfDataset result = JsonLd.toRdf(document).loader(new ClasspathLoader()).get();

        assertNotNull(result);

        try (final InputStream is = getClass().getResourceAsStream("/com/hasmac/jsonld/test/issue63-out.nq")) {
    ...
    void testPerformance() throws JsonLdError, IOException {
        ...
        final Document document = readDocument("/com/hasmac/jsonld/test/issue62-in.json");
        assertNotNull(document);

        final Document expected = readDocument("/com/hasmac/jsonld/test/issue62-out.json");
